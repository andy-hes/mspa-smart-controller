...
uint8_t lastStatus0E = 0xFF;
uint8_t lastStatus15 = 0xFF;
...
void logFrame(const char* dir, uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3, bool valid) {
  ...
  switch (b1) {
    ...
    case 0x0E:
      name = "ozone";
      snprintf(note, sizeof(note), "ozone=%s", b2 ? "on" : "off");
      break;
    case 0x15:
      name = "uvc";
      snprintf(note, sizeof(note), "uvc=%s", b2 ? "on" : "off");
      break;
    case 0x16:
      name = "heartbeat";
...
void setupRoutes() {
  server.on("/api/status", HTTP_GET, [](AsyncWebServerRequest* req) {
    ...
    doc["last_status_0e"] = lastStatus0E;
    doc["last_status_15"] = lastStatus15;
  ...
  server.on("/api/preset/ozone/on", HTTP_POST, [](AsyncWebServerRequest* req) { sendFrame(0x0E, 0x01); req->send(200, "application/json", "{\"ok\":true}"); });
  server.on("/api/preset/ozone/off", HTTP_POST, [](AsyncWebServerRequest* req) { sendFrame(0x0E, 0x00); req->send(200, "application/json", "{\"ok\":true}"); });
  server.on("/api/preset/uvc/on", HTTP_POST, [](AsyncWebServerRequest* req) { sendFrame(0x15, 0x01); req->send(200, "application/json", "{\"ok\":true}"); });
  server.on("/api/preset/uvc/off", HTTP_POST, [](AsyncWebServerRequest* req) { sendFrame(0x15, 0x00); req->send(200, "application/json", "{\"ok\":true}"); });
...
<body>
  ...
  <div class="row">
    ...
    <button onclick="post('/api/preset/ozone/on')">Ozone ON</button>
    <button onclick="post('/api/preset/ozone/off')">Ozone OFF</button>
    <button onclick="post('/api/preset/uvc/on')">UVC ON</button>
    <button onclick="post('/api/preset/uvc/off')">UVC OFF</button>
  ...
  <script>
    ...
    async function refreshStatus(){
      ...
      document.getElementById('status').textContent =
        ...
        ' | 0x0E: ' + s.last_status_0e +
        ' | 0x15: ' + s.last_status_15 +
