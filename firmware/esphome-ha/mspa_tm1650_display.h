...
class MspaTm1650Display {
 public:
  ...
  void displayNumber3(int value, bool decimal_middle = false) {
    ...
    uint8_t d0 = (value >= 100) ? kSeg[hundreds] : 0x00;
    uint8_t d1 = (value >= 10) ? kSeg[tens] : 0x00;
    if (decimal_middle) d1 |= 0x80;
    uint8_t d2 = kSeg[ones];
 ...
 private:
  ...
  static constexpr uint8_t kSeg[10] = {
  ...
  uint8_t encodeChar(char c) {
    switch (c) {
      case '0': return kSeg[0];
      case '1': return kSeg[1];
      case '2': return kSeg[2];
      case '3': return kSeg[3];
      case '4': return kSeg[4];
      case '5': return kSeg[5];
      case '6': return kSeg[6];
      case '7': return kSeg[7];
      case '8': return kSeg[8];
      case '9': return kSeg[9];
      case 'A':
      case 'a': return 0x77;
      case 'B':
      case 'b': return 0x7C;
